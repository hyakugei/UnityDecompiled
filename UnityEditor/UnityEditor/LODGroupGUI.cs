...
namespace UnityEditor
{
	internal static class LODGroupGUI
	{
		public class GUIStyles
		{
			...
			public readonly GUIContent m_UploadToImporter = EditorGUIUtility.TrTextContent("Upload to Importer", "Upload the modified screen percentages to the model importer.", null);

			public readonly GUIContent m_UploadToImporterDisabled = EditorGUIUtility.TrTextContent("Upload to Importer", "Number of LOD's in the scene instance differ from the number of LOD's in the imported model.", null);

			public readonly GUIContent m_RecalculateBounds = EditorGUIUtility.TrTextContent("Recalculate Bounds", "Recalculate bounds to encapsulate all child renderers.", null);

			public readonly GUIContent m_RecalculateBoundsDisabled = EditorGUIUtility.TrTextContent("Recalculate Bounds", "Bounds are already up-to-date.", null);

			public readonly GUIContent m_LightmapScale = EditorGUIUtility.TrTextContent("Recalculate Lightmap Scale", "Set the lightmap scale to match the LOD percentages.", null);

			public readonly GUIContent m_RendersTitle = EditorGUIUtility.TrTextContent("Renderers:", null, null);

			public readonly GUIContent m_AnimatedCrossFadeInvalidText = EditorGUIUtility.TrTextContent("Animated cross-fading is currently disabled. Please enable \"Animate Between Next LOD\" on either the current or the previous LOD.", null, null);

			public readonly GUIContent m_AnimatedCrossFadeInconsistentText = EditorGUIUtility.TrTextContent("Animated cross-fading is currently disabled. \"Animate Between Next LOD\" is enabled but the next LOD is not in Animated Cross Fade mode.", null, null);

			public readonly GUIContent m_AnimateBetweenPreviousLOD = EditorGUIUtility.TrTextContent("Animate Between Previous LOD", "Cross-fade animation plays when transits between this LOD and the previous (lower) LOD.", null);
		}
