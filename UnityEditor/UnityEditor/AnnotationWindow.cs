...
namespace UnityEditor
{
	internal class AnnotationWindow : EditorWindow
	{
		...
		private static AnnotationWindow s_AnnotationWindow = null;
		...
		private static float ConvertTexelWorldSizeTo01(float texelWorldSize)
		{
			...
			{
				result = (Mathf.Log10(texelWorldSize) - -3f) / 3f;
			}
			return result;
		}
		...
		private static float Convert01ToTexelWorldSize(float value01)
		{
			...
			{
				result = Mathf.Pow(10f, -3f + 3f * value01);
			}
			return result;
		}
		...
		private void IconHasChanged()
		{
			if (!string.IsNullOrEmpty(this.m_LastScriptThatHasShownTheIconSelector))
			{
				...
				{
					if (current.m_ScriptClass == this.m_LastScriptThatHasShownTheIconSelector)
					{
						if (!current.m_IconEnabled)
						{
		...
		private AInfo GetAInfo(int classID, string scriptClass)
		{
			...
			if (scriptClass != "")
			{
				result = this.m_ScriptAnnotations.Find((AInfo o) => o.m_ScriptClass == scriptClass);
			}
			else
			{
				result = this.m_BuiltinAnnotations.Find((AInfo o) => o.m_ClassID == classID);
			}
			return result;
		}
		...
		{
			GUI.Label(new Rect(1f, 0f, base.position.width - 2f, topSectionHeight), "", EditorStyles.inspectorBig);
			float num = 7f;
			float num2 = 11f;
			float num3 = num;
			float num4 = 120f;
			...
			if (AnnotationWindow.s_Debug)
			{
				Rect position2 = new Rect(0f, num3 + 10f, base.position.width - num2, num5);
			...
			using (new EditorGUI.DisabledScope(!AnnotationUtility.use3dGizmos))
			{
				...
				if (GUI.changed)
				{
					AnnotationUtility.iconSize = AnnotationWindow.Convert01ToTexelWorldSize(num7);
			...
			if (listElements.Count > 0)
			{
				...
				if (useSeperator)
				{
					...
					if (doDraw)
					{
						...
						GUI.Label(new Rect(1f, num, listElementWidth - 2f, num2), GUIContent.Temp(""), style2);
						GUI.Label(new Rect(10f, num + 3f, listElementWidth - 15f, 3f), GUIContent.Temp(""), AnnotationWindow.m_Styles.seperator);
					}
					num += num2;
				}
			}
