...
namespace UnityEngine.Experimental.UIElements
{
	...
	public class VisualTreeAsset : ScriptableObject
	{
			...
			if ((this.m_VisualElementAssets != null && this.m_VisualElementAssets.Count > 0) || (this.m_TemplateAssets != null && this.m_TemplateAssets.Count > 0))
			{
				...
				if (dictionary.TryGetValue(0, out list2) && list2 != null)
				{
					foreach (VisualElementAsset current in list2)
					{
						...
						VisualElement child = this.CloneSetupRecursively(current, dictionary, new CreationContext(slotInsertionPoints, this, target));
		...
		{
			return this.m_Slots != null && this.m_Slots.Exists((VisualTreeAsset.SlotDefinition s) => s.name == slotName);
		}
		...
		internal void RegisterUsing(string alias, string path)
		{
			...
			int num = 0;
			while (num < this.m_Usings.Count && alias.CompareTo(this.m_Usings[num].alias) != -1)
			{
				num++;
			}
			this.m_Usings.Insert(num, new VisualTreeAsset.UsingEntry(alias, path));
		}
	}
}
