...
namespace UnityEngine.Experimental.UIElements
{
	internal class Panel : BaseVisualElementPanel
	{
		...
		{
			int num = 0;
			while (this.visualTree.IsDirty(ChangeType.PersistentData | ChangeType.PersistentDataPath))
			{
		...
		public void PaintSubTree(Event e, VisualElement root, Matrix4x4 offset, Rect currentGlobalClip)
		{
			if ((root.pseudoStates & PseudoStates.Invisible) != PseudoStates.Invisible)
			{
				...
				{
					if (!this.ComputeAAAlignedBound(root.worldBound, offset).Overlaps(currentGlobalClip))
					{
						return;
					}
				}
				if ((root.panel.panelDebug == null || !root.panel.panelDebug.RecordRepaint(root)) && root.usePixelCaching && this.allowPixelCaching && root.worldBound.size.magnitude > Mathf.Epsilon)
				{
		...
		public override void Repaint(Event e)
		{
			...
			this.PaintSubTree(e, this.visualTree, Matrix4x4.identity, this.visualTree.layout);
			if (base.panelDebug != null)
			{
				GUIClip.SetTransform(Matrix4x4.identity, new Rect(0f, 0f, this.visualTree.style.width, this.visualTree.style.height));
